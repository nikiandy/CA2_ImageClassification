...
def main():
    ...
    model = tf.keras.Model(inputs, outputs, name="chest_xray_transfer")
    ...
    if fit:
        ...
        if not MODEL_PATH.exists():
            raise FileNotFoundError('No saved model at "{}". Train first with fit=True.'.format(MODEL_PATH))
    ...
    if history is not None:
        ...
        ax.set_title('Model accuracy (train vs val - overfitting check)')
        ax.set_xlabel('Epoch')
        ...
        ax.set_title('Model loss (train vs val - overfitting check)')
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
    ...
    print("\nClassification report (test set) - precision / recall / F1:")
